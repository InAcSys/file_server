...
class FixedOnWorkerStart
{
     ...
     * 
     * FIXED: This method now returns false to avoid calling config() before
     * the application is fully bootstrapped, which was causing the error.
     *
     * @return bool
     */
    protected function shouldClearOpcodeCache()
    {
        // Always return false to prevent "Class 'config' does not exist" error
        // during worker startup when Laravel application is not yet fully bootstrapped
        return false;
    }
